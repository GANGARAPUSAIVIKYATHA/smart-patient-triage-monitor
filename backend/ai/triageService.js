...
const analyzeTriage = (input) => {
  const normalizedSymptoms = (input.symptoms || "").toLowerCase();
  const normalizedConditions = (input.preExistingConditions || "").toLowerCase();
  ...
  const confidenceScore = Math.min(98, Math.max(62, 64 + score * 4));

  return {
    riskLevel,
    recommendedDepartment,
    confidenceScore,
    explanation: explanations.length > 0 ? Array.from(new Set(explanations)).slice(0, 4) : ["No critical risk markers detected."],
    urgentAlert: riskLevel === 'HIGH'
  };
};
